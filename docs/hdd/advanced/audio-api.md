...
??? warning "Exclusive无法启动游戏"
    ...
    在 `高级` 选项卡里，将默认格式改成 `44100Hz, 16bit` ，SDVX EG通常采用 `48000Hz, 24bit` `48000Hz, 16bit` 运行是没有问题的，但不排除一些兼容性问题
...
!!! info ""
    ...
    ASIO是SDVX女武神官机默认的输出模式，基板内置华硕Xonar AE独立声卡
    
    ASIO其需要硬件支持才可使用，例如较新的Realtek声卡，以及各种另外加的独立声卡
...
!!! info ""
    ...
    - ASIO支持调整缓冲区，能最低调到16
    - 支持多通道输出
        - 录音/直播声卡大部分ASIO只有双通道（例如Focusrite，YAMAHA），不过现在可以在spice里打补丁强制2通道输出，所以也可以用，但是无法使用游戏内的耳机孔输出，有需求的请自行斟酌
